...
# Slack webhook URL - Store in Airflow Connections for security
slack_conn = BaseHook.get_connection('slack_webhook_url')
SLACK_WEBHOOK_URL = json.loads(slack_conn.extra).get('webhook_token')
...
def get_snowflake_config() -> Dict:
    ...
    try:
        ...
        return {
            ...
            'database': extra.get('database', 'STOCKS'),
...
def check_consumer_health(**kwargs):
    ...
    try:
        ...
        try:
            ...
            if is_market_open() and age_seconds > MAX_AGE:
                error_msg = f"""
                ...
                **Action**: SSH to consumer server and restart consumer.py
                ...
                logger.info(f"✓ Market is not open.")
...
def download_and_load_to_snowflake(**kwargs):
    ...
    # This is used to prevent duplicate loads during the MinIO-to-Snowflake transfer
    ...
    # The lock (a threading.Lock() object) ensures only one thread at a time can modify these shared lists, 
    ...
    # Creating a new thread is expensive (time + memory). It involves OS-level operations, allocating stack space, etc.
    # Destroying threads also costs resources. Thread pool reuses existing threads: avoids this overhead.
    # Thread pool caps the number of concurrent threads: prevents system overload while allowing parallelism.
    with ThreadPoolExecutor(max_workers=max_workers) as executor:

        # A Future object is a placeholder (or "promise") for a result that will be available later.
        # When you call executor.submit(func, args), it immediately returns a Future object.
        # future_to_metadata is a dictionary that maps each Future to the metadata that was used to create it.
        # Key: The Future object returned by executor.submit(...), Value: The metadata dictionary (info about the file being uploaded)
        future_to_metadata = {
            executor.submit(upload_single_file, metadata): metadata for metadata in file_metadata_list
        }
        
        # as_completed() is a helper function from concurrent.futures that yields Future objects as soon as they finish, 
